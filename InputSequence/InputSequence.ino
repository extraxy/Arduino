...
const byte steps[numSteps] = {0, 1, 1, 2, 2, 3, 3, 0, 0, 0,}; // i.e. press button #2 once, then button #3 twice, then button #0, then button #1.
// These pins are used to light up LEDs to show the player's progress, so one output pin per step in the puzzle.
const byte ledPins[numSteps] = {12, 11, 10, 9, 8, 12, 11, 10, 9, 8};
